...
function Services({ modeToggler }) {
    const { data, error } = useSWR('/api/portfolio', fetcher);const popUpAnimation={offscreen:{opacity:0,scale:0},onscreen:{opacity:1,scale:1},transition:{duration:.4,delay:.1}},mainHeadingAnimation={offscreen:{opacity:0,x:-50},onscreen:{x:0,opacity:1}};if (error) return <div>Failed to load</div>;if (!data) return <div>Loading...</div>
    return (
